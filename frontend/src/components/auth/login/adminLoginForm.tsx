...
import { adminSiginApi } from "../../../api/auth/authServices";
import { toast } from "react-toastify";
import { useState } from "react";
import { setAccessToken } from "../../../store/slices/auth/adminAuthSlice";
...
const adminLoginForm: React.FC = () => {
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            const response = await adminSiginApi({
                email: values.email,
                password: values.password,
                role: "admin"
            });
            console.log("under the api")
            // const accessToken = response?.data?.accessToken;
            // const admin = response?.data?.UserDeepCopy;
            // if (!accessToken ) {
