...
const ContestResultsModal: React.FC<ContestResultsModalProps> = ({
  ...
  const getRankBadge = (rank: number) => {
    switch (rank) {
      case 1:
        return <div className="w-8 h-8 rounded-full bg-yellow-100 text-yellow-600 flex items-center justify-center font-bold shadow-sm border border-yellow-200"><Trophy className="w-4 h-4" /></div>;
      case 2:
        return <div className="w-8 h-8 rounded-full bg-gray-200 text-gray-600 flex items-center justify-center font-bold shadow-sm border border-gray-300"><Medal className="w-4 h-4" /></div>;
      case 3:
        return <div className="w-8 h-8 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center font-bold shadow-sm border border-amber-200"><Medal className="w-4 h-4" /></div>;
      default:
        return <div className="w-8 h-8 rounded-full bg-orange-50 text-[#D0864B] flex items-center justify-center font-bold shadow-sm border border-orange-100 text-sm">{rank}</div>;
    }
  };
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-[#FDFBF7]">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-dashed border-gray-200">
              ...
              <p className="text-gray-500 text-sm">It looks like no one participated or the results are not ready yet.</p>
            </div>
          ) : (
