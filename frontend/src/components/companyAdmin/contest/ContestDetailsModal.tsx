            ...
            <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-sm text-gray-500">{contestData.description || 'Test your typing speed and accuracy in this exciting competition'}</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                    ...
                    <div className="bg-gray-50 rounded-xl p-6 grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                        ...
                        <div>
                            ...
                            <span className="text-sm font-bold text-gray-900">{contestData.participantsCount} / {contestData.maxParticipants}</span>
                        </div>
                        <div>
                            <span className="text-xs font-medium text-gray-400 mb-1 block">Reward</span>
                            <span className="text-sm font-bold text-gray-900">{typeof contestData.reward === 'number' ? `$${contestData.reward}` : contestData.reward}</span>
                        </div>
                        <div>
                            ...
                            <div className="divide-y divide-gray-100 max-h-60 overflow-y-auto custom-scrollbar">
                                {participantsList && participantsList.length > 0 ? (
                                    participantsList.map((participant, idx) => (
                                        <div key={idx} className="px-4 py-3 grid grid-cols-2 text-sm items-center hover:bg-gray-50 transition-colors">
                ...
                <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex gap-3">
                    <button onClick={() => setIsEditModalOpen(true)} className="px-5 py-2.5 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition-colors shadow-sm flex items-center gap-2">
