...
import { ArrowLeft, Pause, Square, RotateCcw, MessageSquare, Download, Send, Trophy } from 'lucide-react';
...
const LiveMonitorModal: React.FC<LiveMonitorModalProps> = ({
    ...
    }, [contestDetails?.startTime, contestDetails?.duration, contestDetails?.countDown]);
                        ...
                        {contestStatus === 'completed' || finalResult.length > 0 ? (
                            <>
                                ...
                                <div className="flex flex-col gap-4">
                                    {finalResult
                                        ...
                                        .map((result, index) => (
                                            <div key={result.userId} className="bg-gray-50/50 rounded-2xl p-4 border border-gray-100 flex items-center justify-between hover:bg-white hover:shadow-sm transition-all">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <img src={result.imageUrl} className="w-10 h-10 rounded-full border-2 border-white shadow-sm object-cover" alt={result.name} />
                                                    <div>
                                                ...
                                                <div className="flex gap-6 text-right">
                                                    <div className="flex flex-col">
                                                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-1">WPM</span>
                                                        <span className="text-lg font-black text-indigo-600 leading-none">{result.wpm}</span>
                                                    </div>
                                                    <div className="flex flex-col">
                                                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-1">Accuracy</span>
                                                        <span className="text-lg font-bold text-emerald-500 leading-none">{result.accuracy}%</span>
                                                    </div>
                                                    <div className="flex flex-col">
                                                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-1">Errors</span>
                                                        <span className="text-lg font-bold text-red-500 leading-none">{result.errors}</span>
                                                    </div>
                                                    <div className="flex flex-col">
                                                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-1">Prize</span>
                            ...
                            <>
                                <h2 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
                                    Participants <span className="text-gray-500 text-lg font-bold tracking-tight">( {participants.length} )</span>
                                </h2>

                                {participants.length > 0 ? (
                                    <div className="flex flex-col gap-4">
                                        {participants.map((p, idx) => (
                                            <div key={idx} className="bg-gray-50/50 rounded-2xl p-6 border border-gray-100 flex flex-col gap-4 hover:bg-white hover:shadow-sm transition-all">
                                                <div className="grid grid-cols-4 gap-4 items-center">
                                                    <div>
                                                        <p className="font-bold text-gray-900 text-sm mb-2">{p.name || `Player ${idx + 1}`}</p>
                                                        <div className="flex flex-col">
                                                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-1">WPM</span>
                                                            <span className="text-xl font-black text-indigo-600 leading-none">{p.wpm || '0'}</span>
                                                        </div>
                                                    </div>
                                                    <div>
                                                        <div className="flex flex-col mt-7">
                                                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-1">Accuracy</span>
                                                            <span className="text-lg font-bold text-emerald-500 leading-none">{p.accuracy || '0'}%</span>
                                                        </div>
                                                    </div>
                                                    <div>
                                                        <div className="flex flex-col mt-7">
                                                    ...
                                                    <div className="text-right">
                                                        <div className="flex flex-col mt-7 items-end">
                                                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-1">Progress</span>
                                                            <span className="text-lg font-bold text-purple-600 leading-none">{p.progress || '0'}%</span>
                                                        </div>
                                                    </div>
                                                </div>
