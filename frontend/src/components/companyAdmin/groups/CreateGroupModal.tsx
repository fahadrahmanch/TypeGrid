          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className={`w-full px-4 py-3 rounded-xl bg-gray-50 border transition-all text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 ${
                errors.groupName ? "border-red-500 focus:border-red-500" : "border-gray-200 focus:border-blue-500"
              }`}
            />
            {errors.groupName && <p className="text-xs text-red-500 font-medium">{errors.groupName}</p>}
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-6">
              <label className="flex items-center gap-2 cursor-pointer group">
                <div
                  className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${groupingMethod === "auto" ? "border-blue-600" : "border-gray-300 group-hover:border-blue-400"}`}
                >
              ...
              <label className="flex items-center gap-2 cursor-pointer group">
                <div
                  className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${groupingMethod === "manual" ? "border-blue-600" : "border-gray-300 group-hover:border-blue-400"}`}
                >
          ...
          <div className="bg-gray-50 rounded-xl p-6 border border-gray-100 transition-all duration-300">
            {groupingMethod === "auto" ? (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    />
                    {errors.minWpm && <p className="text-[10px] text-red-500">{errors.minWpm}</p>}
                  </div>
                  <div className="space-y-1.5">
                    ...
                    />
                    {errors.maxWpm && <p className="text-[10px] text-red-500">{errors.maxWpm}</p>}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    />
                    {errors.minAccuracy && <p className="text-[10px] text-red-500">{errors.minAccuracy}</p>}
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
                      Max Accuracy (%)
                    ...
                    />
                    {errors.maxAccuracy && <p className="text-[10px] text-red-500">{errors.maxAccuracy}</p>}
                  </div>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <div className="max-h-48 overflow-y-auto pr-2 space-y-2 custom-scrollbar">
                  {users.map((user: any) => (
                    <label
                      ...
                      className="flex items-start gap-3 p-3 bg-white rounded-lg border border-gray-200 cursor-pointer hover:border-blue-200 transition-colors"
                    >
                      <div
                        className={`mt-0.5 w-4 h-4 rounded border flex items-center justify-center transition-colors ${selectedUsers.includes(user._id || user.id) ? "bg-blue-600 border-blue-600" : "border-gray-300"}`}
                      >
                ...
                {errors.users && <p className="text-xs text-red-500 font-medium mt-2 text-center">{errors.users}</p>}
              </div>
            )}
          </div>
        </div>
