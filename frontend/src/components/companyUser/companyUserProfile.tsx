    ...
    <div className="min-h-screen bg-[#FFF8EA] pb-12">
      ...
      <div className="pt-24 px-4 sm:px-8 max-w-7xl mx-auto">
        ...
        <div className="px-4 sm:px-12 flex flex-col sm:flex-row items-end gap-6 mb-12 relative z-10">
          ...
          <div className="flex-1 pb-2 text-center sm:text-left">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-1">
              <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight">{profile.identity.fullName}</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-12">
          <StatCard icon={Zap} label="Avg. Speed" value={`${profile.stats.avgSpeed} WPM`} color="bg-orange-100 text-orange-600" />
          <StatCard icon={Target} label="Accuracy" value={`${profile.stats.accuracy}%`} color="bg-blue-100 text-blue-600" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-3 p-4 bg-white border border-gray-100 rounded-2xl">
                    ...
                    <input type="text" value={profile.identity.fullName} readOnly={!isEditing} className="bg-transparent outline-none w-full font-semibold text-gray-700 disabled:text-gray-500" />
                  </div>
                </div>
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-3 p-4 bg-white border border-gray-100 rounded-2xl">
                    ...
                    <input type="email" value={profile.identity.email} readOnly={!isEditing} className="bg-transparent outline-none w-full font-semibold text-gray-700 disabled:text-gray-500" />
                  </div>
                </div>
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-3 p-4 bg-gray-50 border border-gray-100 rounded-2xl">
                    ...
                    <input type="text" value={profile.identity.role} readOnly className="bg-transparent outline-none w-full font-semibold text-gray-400" />
                  </div>
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-gray-400 capitalize px-1 tracking-widest">Member Since</label>
                  <div className="flex items-center gap-3 p-4 bg-gray-50 border border-gray-100 rounded-2xl">
                    <Calendar className="w-5 h-5 text-gray-400" />
                    <input type="text" value={profile.identity.memberSince} readOnly className="bg-transparent outline-none w-full font-semibold text-gray-400" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#8B7355]/5 border border-[#8B7355]/10 p-8 rounded-[2.5rem] flex flex-col md:flex-row items-center gap-6">
               ...
               <div className="flex-1 text-center md:text-left">
                  ...
                  <p className="text-gray-600 font-medium">Keep your account secure by regularly updating your password.</p>
               </div>
               <button 
                onClick={() => setShowPasswordForm(!showPasswordForm)}
                className={`px-6 py-3 rounded-2xl font-bold transition-all shadow-lg hover:scale-105 active:scale-95 ${
                  showPasswordForm ? "bg-white text-gray-600 border border-gray-200" : "bg-[#8B7355] text-white shadow-[#8B7355]/30 hover:bg-[#725e46]"
                }`}
               >
                ...
                <form onSubmit={handlePasswordSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-1 md:col-span-2">
                      <label className="text-xs font-bold text-gray-400 capitalize px-1 tracking-widest">Current Password</label>
                      <div className="relative group">
                        <input 
                      ...
                      {formErrors.oldPassword && <p className="text-red-500 text-xs mt-1 ml-1 font-medium">{formErrors.oldPassword}</p>}
                    </div>
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-gray-400 capitalize px-1 tracking-widest">New Password</label>
                      <div className="relative group">
                        <input 
                      ...
                      {formErrors.newPassword && <p className="text-red-500 text-xs mt-1 ml-1 font-medium">{formErrors.newPassword}</p>}
                    </div>
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-gray-400 capitalize px-1 tracking-widest">Confirm New Password</label>
                      <div className="relative group">
                        <input 
                      ...
                      {formErrors.confirmPassword && <p className="text-red-500 text-xs mt-1 ml-1 font-medium">{formErrors.confirmPassword}</p>}
                    </div>
                  </div>
          ...
          <div className="space-y-8">
            <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-8 rounded-[2.5rem] shadow-xl text-white relative overflow-hidden group">
               ...
               <p className="text-indigo-100 text-sm mb-6 opacity-90">Improve your typing speed and accuracy with customized lessons.</p>
