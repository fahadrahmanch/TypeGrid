...
const Achievements: React.FC = () => {
    ...
    const getRarityStyles = (rarity: string) => {
        switch (rarity) {
            case 'common': return 'bg-yellow-100 text-yellow-700';
            case 'rare': return 'bg-orange-100 text-orange-700';
            case 'epic': return 'bg-purple-100 text-purple-700';
            case 'legendary': return 'bg-green-100 text-green-700';
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-6 mb-10">
                    <div className="bg-gray-200/40 backdrop-blur-sm p-1.5 rounded-[22px] flex items-center gap-1 w-full sm:w-auto">
                        <button
                            ...
                            className={`flex-1 sm:flex-none px-8 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-2 ${activeTab === 'all'
                        ...
                        <button
                            ...
                            className={`flex-1 sm:flex-none px-8 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-2 ${activeTab === 'unlocked'
                                    ? "bg-[#8B7355] text-white shadow-lg shadow-[#8B7355]/20"
                                    : "text-gray-500 hover:text-gray-700"
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`flex-1 sm:flex-none px-8 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-2 ${activeTab === 'locked'
                                    ? "bg-[#8B7355] text-white shadow-lg shadow-[#8B7355]/20"
                                    : "text-gray-500 hover:text-gray-700"
                                }`}
                        >
                    ...
                    {filteredAchievements.map((ach) => (
                        ...
                        >
                            <div className="flex justify-between items-start mb-6">
                                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner transition-transform duration-500 group-hover:rotate-3 ${ach.isUnlocked ? 'bg-yellow-50' : 'bg-gray-100'}`}>
                                    <img 
                                        src={ach.imageUrl} 
                                        alt={ach.title} 
                                ...
                                <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${getRarityStyles(getRarity(ach.xp))}`}>
                                    {getRarity(ach.xp)}
                                </span>
                            </div>

                            <div className="flex-1">
                                <h3 className={`text-lg font-bold mb-2 transition-colors text-gray-900`}>
                                    {ach.title}
                                </h3>
                                <p className="text-sm text-gray-500 leading-relaxed mb-4 font-medium line-clamp-2">
                                    {ach.description}
                                </p>
