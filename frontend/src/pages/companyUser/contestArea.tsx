                ...
                {finalResult.length > 0 ? (
                    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-8 animate-in fade-in zoom-in duration-500 w-full">
                        ...
                        <div className="w-full max-w-2xl bg-white/90 backdrop-blur-md rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 overflow-hidden relative">
                            ...
                            <div className="divide-y divide-slate-100">
                                {finalResult
                                    ...
                                    .map((result, index) => (
                                        <div key={result.userId} className={`grid grid-cols-[80px_1fr_100px_100px_100px_80px] gap-4 p-4 items-center hover:bg-[#FFF4EC]/50 transition-colors
                                            ${result.userId === user?._id ? "bg-[#FFF4EC]/80" : ""}
                                        `}>
                                            <div className="flex justify-center">
                                                {result.rank === 1 ? (
                                                    <div className="w-8 h-8 rounded-full bg-[#ECA468]/20 text-[#D0864B] flex items-center justify-center font-black shadow-sm ring-2 ring-[#FADDB8]">1</div>
                                                ) : result.rank === 2 ? (
                                                    <div className="w-8 h-8 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center font-black shadow-sm ring-2 ring-slate-200">2</div>
                                                ) : result.rank === 3 ? (
                                                    <div className="w-8 h-8 rounded-full bg-[#ECA468]/10 text-[#D0864B] flex items-center justify-center font-black shadow-sm ring-2 ring-[#ECA468]/30">3</div>
                                                ) : (
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div className="font-bold text-slate-800 text-sm">{result.name} {result.userId === user?._id && "(You)"}</div>
                                                </div>
                                            </div>
                        ...
                        <div className="flex items-center gap-4 mt-4">
                            <button onClick={() => navigate("/company/user/contests")} className="px-6 py-3 rounded-xl bg-white border border-slate-200 text-slate-600 font-bold hover:bg-slate-50 hover:border-slate-300 transition-all flex items-center gap-2">
                                Return to Contests
                            </button>
                        </div>
                    </div>
                                    ...
                                    {isFinished && (
                                        <div className="flex items-center gap-3 bg-[#FFF4EC] text-[#D0864B] px-4 py-2 rounded-xl border border-[#FADDB8] shadow-sm animate-in fade-in slide-in-from-right-4 duration-500">
                                            ...
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[10px] uppercase font-bold tracking-wider opacity-80 mt-0.5">Waiting for others...</span>
                                            </div>
                                        </div>
                                    )}
                                </div>
