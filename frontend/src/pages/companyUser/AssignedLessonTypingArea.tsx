...
const getCharClass = (index: number) => {
  ...
  useEffect(() => {
    ...
    async function saveResult() {
      ...
      try {
        await saveLessonResult(assignedLesson?.id!, {
          wpm,
          accuracy,
          errors,
          totalTyped,
        ...
        >
          ...
          <div className="bg-[#FEFCE8] rounded-[1.3rem] p-8 md:p-12 min-h-[300px] border border-orange-50/50 relative overflow-hidden">
            ...
            <div className="flex justify-between items-center mb-8 border-b border-orange-100/50 pb-4">
              ...
              <button
                onClick={resetTest}
                className="flex items-center gap-2 px-4 py-2 bg-[#B09886] text-white rounded-xl hover:bg-[#967d6c] transition-all hover:scale-105 active:scale-95 text-sm font-semibold shadow-sm"
              >
            ...
            {isFinished && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/90 backdrop-blur-sm z-30 transition-all duration-500 animate-in fade-in">
                {(() => {
                  const isPassed =
                    !isTimeUp &&
                    wpm >= (assignedLesson?.lesson.wpm || 0) &&
                    accuracy >= (assignedLesson?.lesson.accuracy || 0);
