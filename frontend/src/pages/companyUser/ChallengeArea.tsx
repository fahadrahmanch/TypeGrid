   ...
   useEffect(() => {
           ...
           const interval = setInterval(() => {
               ...
               if (elapsed < challengeData.countDown) {
                   ...
                   setPhase("PLAY");
                   setRemainingTime(challengeData.countDown + challengeData.duration - elapsed);
       ...
       }, [challengeData?.startedAt, challengeData?.duration, challengeData?.countDown, isFinished]);
 ...
 const formatTime = (seconds: number) => {
       ...
       useEffect(() => {
        ...
        const renderTextWithHighlight = () => {
            ...
            return challengeData?.lesson?.text.split("").map((char: string, index: number) => {
        ...
        useEffect(() => {
          ...
          useEffect(() => {
              ...
              //handle key down
                ...
                }, [challengeData?.lesson.text, isFinished, typedText, phase, hasError, challengeData?.id]);
                ...
                <div className="flex-1 flex flex-col items-center px-4 md:px-8 max-w-7xl mx-auto w-full gap-3 md:gap-4 py-3 md:py-4 overflow-hidden pointer-events-auto">
                    ...
                    <div className="flex justify-between items-center w-full shrink-0 mb-1 px-2 md:px-0">
                        <div className="text-left">
                            <h1 className="text-2xl md:text-3xl font-extrabold text-[#111827] mb-0.5 tracking-tight">Live Challenge</h1>
                        ...
                        <div className="bg-white px-3 md:px-4 py-1.5 md:py-2 rounded-[1rem] border border-[#FDE6C6] shadow-sm flex items-center gap-2">
                            ...
                            <span className="text-lg md:text-xl font-mono font-bold text-gray-800">{elapsedTime}</span>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-3 md:gap-4 w-full shrink-0">
                        {players && players.map((player, index) => (
                            <div key={index} className="flex-1 bg-white border border-[#FDE6C6] rounded-[1.25rem] md:rounded-[1.5rem] p-4 shadow-sm relative overflow-hidden">
                                ...
                                <div className="flex items-center gap-3 mb-3 relative z-10">
                                    <div className="relative">
                                        <img src={player.imageUrl} alt="Avatar" className="w-12 h-12 rounded-xl border-2 border-orange-100 bg-orange-50 object-cover" />
                                ...
                                <div className="grid grid-cols-4 gap-2 md:gap-3 mb-3 relative z-10">
                                    ...
                                    <div className="bg-[#FFFDF9] rounded-xl p-1.5 md:p-2 border border-[#FDE6C6] flex flex-col items-center justify-center">
                                        ...
                                        <span className="text-lg md:text-xl font-extrabold text-gray-900 leading-none">{player.accuracy}<span className="text-[9px] text-gray-400 font-bold">%</span></span>
                                    </div>
                                    <div className="bg-[#FFFDF9] rounded-xl p-1.5 md:p-2 border border-[#FDE6C6] flex flex-col items-center justify-center">
                                        <div className="flex items-center gap-1 text-red-500 mb-0.5 font-semibold text-[9px] md:text-[10px]">
                                            <AlertCircle className="w-3 h-3" /> Err
                                        </div>
                                        <span className="text-lg md:text-xl font-extrabold text-gray-900 leading-none">{player.errors}</span>
                                    </div>
                                    <div className="bg-[#FFFDF9] rounded-xl p-1.5 md:p-2 border border-[#FDE6C6] flex flex-col items-center justify-center">
                                        <div className="flex items-center gap-1 text-blue-500 mb-0.5 font-semibold text-[9px] md:text-[10px]">
                                            <Clock className="w-3 h-3" /> Time
                                        </div>
                                        <span className="text-lg md:text-xl font-extrabold text-gray-900 leading-none">{player.timeTaken || "00:00"}</span>
                                    </div>
                                </div>
                                ...
                                <div className="relative z-10">
                                    <div className="flex justify-between items-center mb-1">
                                        ...
                                        <span className="text-[9px] md:text-[10px] font-bold text-orange-500">{player.progress}%</span>
                                    </div>
                                    <div className="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden shadow-inner flex">
                                        <div className={`${player.color} h-1.5 rounded-full transition-all duration-500 relative`} style={{ width: `${Math.max(player.progress, 5)}%` }}>
                    ...
                    <div className="w-full bg-white rounded-[1.25rem] md:rounded-[1.5rem] border border-[#FDE6C6] p-4 shadow-sm flex flex-col gap-3 relative mt-0 flex-1 shrink min-h-0 overflow-hidden">
                        ...
                        <div ref={snippetContainerRef} className="relative z-10 text-[#4A5568] text-base md:text-lg leading-[1.6] font-mono tracking-wide flex-1 min-h-[50px] overflow-hidden pr-2">
