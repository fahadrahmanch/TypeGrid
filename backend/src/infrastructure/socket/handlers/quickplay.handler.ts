  ...
  socket.on(
    ...
    async ({ userId, name, imageUrl, timeTaken, totalTyped, errors, typedLength }) => {
      ...
      if (isEnd) {
        ...
        await redis.del(key);
        await injectQuickSocketController.saveQuickPlayResult(gameId, resultArray);
      }
    }
  );

  socket.on("time-up-quick", async ({ userId, name, imageUrl, timeTaken, totalTyped, errors, typedLength }) => {
    const gameId = socket.data.quickGameId;
    if (!gameId) return;
    const key = `quick:game:${gameId}`;
    const raw = await redis.hget(key, userId);
