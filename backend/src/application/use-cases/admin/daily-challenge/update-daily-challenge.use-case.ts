...
export class UpdateDailyAssignChallengeUseCase implements IUpdateDailyAssignChallengeUseCase {
  ...
  async execute(id: string, data: { challengeId?: string; date?: Date }): Promise<DailyAssignChallengeResponseDTO | null> {
    const existing = await this._dailyAssignChallengeRepository.findById(id);
    if (!existing) return null;

    if (data.date) {
      const isExist = await this._dailyAssignChallengeRepository.findOne({ date: data.date, _id: { $ne: id } });
      if (isExist) {
