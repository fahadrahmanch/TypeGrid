...
export class CreateDailyAssignChallengeUseCase implements ICreateDailyAssignChallengeUseCase {
    ...
    async execute(data: { challengeId: string; date: Date }): Promise<DailyAssignChallengeResponseDTO> {
      const isExist = await this._dailyAssignChallengeRepository.findOne({date:data.date});
      if(isExist){
      ...
      const dailyAssignChallenge = await this._dailyAssignChallengeRepository.create(dailyAssignChallengeEntity.toObject());
