...
export class GetChallengesUseCase implements IGetChallengesUseCase {
    ...
    async execute(searchText: string, page: number, limit: number): Promise<{ challenges: ChallengeResponseDTO[], total: number }> {
        const { challenges, total } = await this._challengeRepository.getChallenges(searchText, page, limit);
        return {
