...
export class GetRewardsUseCase implements IGetRewardsUseCase{
    ...
    async execute(searchText:string,page:number,limit:number): Promise<{rewards:RewardResponseDTO[],total:number}> {
        const {rewards,total} = await this._rewardRepository.getRewards(searchText,page,limit);
        return {rewards:rewards.map((reward) => mapToReward(reward.toObject())),total};
    }
}