...
export class LoginUseCase implements ILoginUseCase {
  ...
  async execute(email: string, password: string, allowedRoles: string[]): Promise<AuthUserEntity> {
    ...
    if (!allowedRoles.includes(user.role)) {
      throw new CustomError(HttpStatusCodes.UNAUTHORIZED, MESSAGES.AUTH_UNAUTHORIZED_ROLE);
    }

    if (!user.password) {
      throw new CustomError(HttpStatusCodes.UNAUTHORIZED, MESSAGES.AUTH_INCORRECT_PASSWORD);
    }
