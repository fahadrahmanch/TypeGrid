...
export class GetPracticeTypingContentUseCase implements IGetPracticeTypingContentUseCase {
  ...
  async execute(level: string, category: string): Promise<PracticeTypingDTO> {
    if (!level || !category) {
      throw new CustomError(HttpStatusCodes.BAD_REQUEST, MESSAGES.LEVEL_AND_CATEGORY_REQUIRED);
    }
    ...
    if (lessons.length === 0) {
      throw new CustomError(HttpStatusCodes.NOT_FOUND, MESSAGES.NO_LESSONS_FOUND_FOR_CRITERIA);
    }
