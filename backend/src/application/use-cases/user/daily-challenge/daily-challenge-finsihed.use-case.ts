...
export class DailyChallengeFinishedUseCase implements IDailyChallengeFinishedUseCase {
    ...
    async execute(userId: string, wpm: number, accuracy: number): Promise<void> {
        ...
        let dailyChallengeProgress = await this._dailyChallengeProgressRepository.findOne({ userId, dailyChallengeId: dailyChallenge.getId(), date: { $gte: startOfDay, $lte: endOfDay } })
