...
export class ValidateGroupPlayMemberUseCase implements IValidateGroupPlayMemberUseCase {
  ...
  async execute(gameId: string, userId: string): Promise<boolean> {
    ...
    const group = await this.groupRepository.findById((competition as any)?.groupId);

    if (!group) {
      return false;
    }

    const isMember = group.getMembers().some((member: any) => member.toString() === userId.toString());
    return isMember;
  }
}
