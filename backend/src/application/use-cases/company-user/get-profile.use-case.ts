...
export class GetProfileUseCase implements IGetProfileUseCase {
  ...
  async getProfile(userId: string): Promise<CompanyUserProfileDTO | null> {
    ...
    const completedLessonsCount = await this.lessonResultRepository.countCompletedLessons(userId);

    const avgSpeed = stats?.getWpm() || 0;
    const accuracy = stats?.getAccuracy() || 0;

    return mapToCompanyUserProfileDTO(user, companyName, avgSpeed, accuracy, completedLessonsCount);
  }
}
