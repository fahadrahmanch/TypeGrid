...
export class saveLessonResultUseCase implements ISaveLessonResultUseCase{
    ...
    async execute(userId:string,assignmentId:string,result:any):Promise<void>{
        ...
        if(existingResult.getStatus()==='completed'){
       ...
       const hy= await this._baseRepositoryLessonAssignment.update({
            _id:assignmentId,
             status:status,
        })
        console.log("hy",hy)
        const saveResult=await this._baseRepositoryLessonResult.update({_id:existingResult._id,$set:saveResultObject})
        if(!saveResult){
            throw new Error("Failed to save lesson result")
        }
    }
