...
export class DailyChallengeController {
  ...
  async getTodayChallenge(
    ...
    try {
      ...
      logger.info("Daily challenge fetched successfully", {
  ...
  async dailyChallengeFinished(
    ...
    try {
      ...
      await this._dailyChallengeFinishedUseCase.execute(
        req.user?.userId!,
        wpm!,
        accuracy!,
      );
  ...
  async getStatistics(
    ...
    try {
      const stats = await this._getDailyChallengeStatsUseCase.execute(
        req.user?.userId!,
      );
      res.status(HttpStatus.OK).json({
        success: true,
